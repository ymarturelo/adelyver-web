...
export const SupabaseClientsController: IClientsController = {
  findClients: async (
    ...
    try {
      ...
      if (error)
        return Result.err({ code: "DB_ERROR", message: error.message });

      let rows = data ?? [];
      if (req.name) {
        rows = rows.filter((r: any) =>
          (r.full_name ?? "").toLowerCase().includes(req.name!.toLowerCase())
        );
      }
      if (req.phone) {
      ...
      if (error instanceof Error) {
        return Result.err({ code: "UNKNOWN_ERROR", message: error.message });
      }
      return Result.err({ code: "UNKNOWN_ERROR", message: "unknown" });
    }
  },
